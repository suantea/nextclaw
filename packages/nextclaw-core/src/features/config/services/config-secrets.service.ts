...
import { isSensitiveConfigPath } from "@core/features/config/utils/config-schema-hints.utils.js";
...
class SecretRuntimeResolver {
  ...
  private getFileSnapshot = (
    alias: string,
    provider: Extract<SecretProviderConfig, { source: "file" }>
  ...
  private getExecSnapshot = (
    alias: string,
    provider: Extract<SecretProviderConfig, { source: "exec" }>
  ): Record<string, unknown> => {
